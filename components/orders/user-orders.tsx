...
export default function UserOrders() {
  ...
  const ordersInProgress = orders?.filter(order =>
    ['EmOrcamento', 'PedidoPendenteSincronizacao', 'PedidoSincronizado', 'PedidoEmTransporte'].includes(order.orderStatus)
  ) ?? []

  const ordersBilled = orders?.filter(order =>
    order.orderStatus === 'PedidoFaturado'
  ) ?? []

  const ordersFinished = orders?.filter(order =>
    ['PedidoEntregue', 'PedidoFinalizado'].includes(order.orderStatus)
  ) ?? []

  const ordersCanceled = orders?.filter(order =>
    order.orderStatus === 'PedidoCancelado'
  ) ?? []
  ...
  return (
    <div className="container min-h-screen mx-auto px-4 pb-6 mt-24  max-w-7xl border-b">
      ...
      <nav className="flex items-center space-x-2 text-sm text-muted-foreground mb-6">
        <Breadcrumb firtsDescription="Usuário" secondDescription="Histórico de pedidos"/>
      </nav>

      {/* Header */}
      <div className="flex justify-between items-start mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 mb-2">Histórico de pedidos</h1>
          <p className="text-gray-600">Acompanhe o andamento de todos os seus pedidos!</p>
        </div>
