...
export function FinishedOrdersTabs({ orders }: Props) {
  return (
    <TabsContent value="concluidos">
      {orders.length === 0 ? (
        <OrdersEmptyState title="Nenhum pedido concluído" description="Você não possui pedidos concluído no período selecionado."/>
          ...
          <table className="w-full">
            <thead className="border-b">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Data de Faturamento</th>
                ...
                <th className="px-6 py-3 sr-only text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Ações</th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-gray-200">
              {orders.map((order) => (
                <tr key={order.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {order.orderDone ? formatDate(order.orderDone) : '---'}
                  </td>
